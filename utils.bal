...
isolated function createUserDetails(datasource:UserInfo[] result) returns UserDetails[] {
    ...
    string[] names = from datasource:UserInfo user in result select user.name;

    foreach string user in names {
        UserDetails[] users = from var {id, name, email, age} in result where name == user select {id, name, email, age};
        userArr.push(...users);
    }
    return userArr;
}
